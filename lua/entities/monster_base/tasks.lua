...
function ENT:TaskStart_PlaySequence( data )
	...
	self.seq_oldnpcstate = self:GetNPCState()
	...
	local function ResetIdle()
		if !ValidEntity( self ) or self:Health() <= 0 then return end
		//self:TaskComplete()
		//if self:HasCondition(10) or self:HasCondition(7) or self:HasCondition(8) or self:HasCondition(9) then
		self:FindInCone( 1, 9999 )
		if ValidEntity(self.enemy) or self.possessed then
			self:SetNPCState( NPC_STATE_ALERT )
		else
			self:SetNPCState( self.seq_oldnpcstate )
		end
		self.seq_oldnpcstate = nil
...
function ENT:Task_PlaySequence( data )
	...
	if ( self.TaskSequenceEnd and CurTime() < self.TaskSequenceEnd ) then return end
