...
function ENT:TurnToEnemyXR(xr_min,xr_max)
	if ValidEntity(self.enemy) then
		local pitch = (self:GetPos() -(self.enemy:GetPos() +self:GetPositionAnomaly(self.enemy:GetClass()))):Angle().p
		if pitch > 270 then pitch = pitch -360 end
		if pitch > xr_max then
			self:SetPoseParameter("XR", xr_max)
		elseif pitch < xr_min then
			self:SetPoseParameter("XR", xr_min)
		else
			self:SetPoseParameter("XR", pitch)
		end
	end
end
...
function ENT:EndPossession()
	...
	if self.master and ValidEntity( self.master ) then self.master:GetTable().frozen = false; self.master.aimvector = nil; self.master:Spawn(); self.master:SetViewEntity( self.master ) end
...
function ENT:CheckEnemy( rel )
	if rel == 1 then
		if ValidEntity( self.enemy ) and self.enemy:Health() > 0 and self:Visible(self.enemy) and ( !self.WaterMonster or ( self.WaterMonster and self.enemy:WaterLevel() > 1 ) ) then
			return true
	...
	if ValidEntity( self.enemy_fear ) and self.enemy_fear:Health() > 0 and self:Visible(self.enemy_fear) and ( !self.WaterMonster or ( self.WaterMonster and self.enemy_fear:WaterLevel() > 1 ) ) then
		return true
...
function ENT:GetEnemies()
	...
	for k, v in pairs(enemies) do
		...
		if ValidEntity(v) and ((v:IsNPC() and v != self) or (v:IsPlayer() and convar_ignoreply == 0 and !self.ignoreplys)) and v:Health() > 0 /*and self:Visible(v)*/ and (disposition == 1 or disposition == 2) and ( !self.WaterMonster or ( self.WaterMonster and v:WaterLevel() > 1 ) ) and ((ang <= 90 and ang >= 0) or (ang <= 360 and ang >= 270)) then
			table.insert(tbl,v)
		end
	end
	return tbl
...
function ENT:FindInCone( searchDist )
	local convar_ignoreply = GetConVarNumber("ai_ignoreplayers")
...
function ENT:GetPositionAnomaly(class)
	if class == "monster_pitdrone" then return Vector(0,0,-10) end
	if class == "npc_fastzombie" or class == "monster_bullchicken" or class == "monster_houndeye" or class == "monster_panthereye" or class == "npc_antlion" or class == "npc_antlion_worker" then return Vector(0,0,-22) end
	if class == "monster_headcrab" or class == "monster_pitdrone" or class == "monster_penguin" or class == "npc_stukabat" or class == "npc_headcrab" or class == "npc_headcrab_fast" or class == "npc_headcrab_black" or class == "npc_headcrab_poison" or class == "npc_fastzombie_torso" or class == "npc_zombie_torso" or class == "npc_cscanner" or class == "npc_manhack" or class == "npc_mortarsynth" or class == "monster_alien_babyvoltigore" then return Vector(0,0,-50) end
	if class == "monster_babycrab" or class == "monster_snark" then return Vector(0,0,-58) end
	if class == "player" and ValidEntity(self.enemy) and self.enemy:IsPlayer() and self.enemy:KeyDown( IN_DUCK ) then return Vector(0,0,-22) end
	...
	/*if !self.playingpreanim then
		local function CheckMovementActivity()
			...
			if self:GetActivity() == 1 then
				...
				if self.sequence and ValidEntity( self.sequence ) then self.sequence:TaskDone( "MoveToPosition" ) end
			end
		end
...
function ENT:TaskStart_ScriptedPlayedPost()
	self:TaskComplete()
	if self.sequence and ValidEntity( self.sequence ) and !self.sequence.InPostAnimation then self.sequence:TaskDone( "PlayPostAnimation" ) end
end
...
function ENT:TaskStart_ScriptedPlayedSequence()
	self:TaskComplete()
	if self.sequence and ValidEntity( self.sequence ) and !self.sequence.InMainAnimation then self.sequence:TaskDone( "PlayMainAnimation" ) end
end
...
function ENT:TaskStart_ScriptedPlayedPre()
	...
	if self.sequence and ValidEntity( self.sequence ) and !self.sequence.InPreAnimation then self.sequence:TaskDone( "PlayPreAnimation" ) end
end
