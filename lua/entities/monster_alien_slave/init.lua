...
ENT.DSounds = {}
ENT.DSounds["Attack"] = {"debris/zap4.wav"}
ENT.DSounds["Alert"] = {"aslave/slv_alert1.wav", "aslave/slv_alert2.wav", "aslave/slv_alert3.wav"}
ENT.DSounds["Death"] = {"aslave/slv_die1.wav", "aslave/slv_die2.wav"}
ENT.DSounds["Pain"] = {"aslave/slv_pain1.wav", "aslave/slv_pain2.wav"}
...
function ENT:Initialize()
	...
	self:SetUpEnemies( {"monster_alien_controller", "monster_alien_grunt", "monster_nihilanth"} )
...
function ENT:Attack_Range( poss )
	...
	self.Sounds["Attack"][1]:Stop()
	self.Sounds["Attack"][1]:Play()
	...
	local function lAttak( tar )
		...
		if poss or self:GetPos():Distance( self.enemy:GetPos() ) < 2000 then
			...
			local function zap()
				...
				if hitentity and ValidEntity(trace.Entity) then
	...
	if( ( self.FoundEnemy or self.FoundEnemy_fear ) and !self.attacking and !self.possessed and convar_ai == 0 ) then
		...
		if( self.enemy and ValidEntity( self.enemy ) and self.enemy:IsValid() and self.enemy:Health() > 0 and self:Disposition( self.enemy ) == 1 and self.enemy:GetPos():Distance( self:GetPos() ) <= self.closest_range ) then
			...
			elseif( self.enemy:GetPos():Distance( Pos ) < self.MeleeDistance and self:HasCondition( 10 ) and !self:HasCondition( 42 ) ) then
				...
				self:UpdateEnemyMemory( self.enemy, self.schedule_runtarget_pos )
				self:AttackMelee(schdMeleeAttack, 75, sk_aslave_slash_value /3, "islave", Angle( -6, -15, 0 ), 0.3, 0, "npc/zombie/claw_strike" ..math.random(1,3).. ".wav", {0.588, "npc/zombie/claw_strike" ..math.random(1,3).. ".wav", Angle( -6, 15, 0 ), 0.915, "npc/zombie/claw_strike" ..math.random(1,3).. ".wav", Angle( -5, -11, 0 )} )
