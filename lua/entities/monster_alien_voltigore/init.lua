...
ENT.Model = "models/voltigore.mdl"
...
function ENT:Initialize()
	...
	if !self.health then
		self:SetHealth(sk_voltigore_health_value)
	end
	
	if self.triggertarget and self.triggercondition == "3" then self.starthealth = self:Health() end

	self:SetUpEnemies({"monster_pitdrone", "monster_shocktrooper", "monster_shockroach", "monster_geneworm", "monster_alien_babyvoltigore", "monster_pitworm"})
...
function ENT:Attack_Range( poss )
	...
	for i = 0,2 do
		...
		beam_target:SetParent(self)
		beam_target:Spawn()
		beam_target:Activate()
		beam_target:Fire("SetParentAttachment", tostring(i), 0)
	
		local beam = ents.Create("env_beam")
		beam:SetPos(self:GetPos() +self:GetForward() *40 +self:GetUp() *30)
		beam:SetParent(self)
		...
		beams[beam] = beam_target
	end

	//self:PlayRandomSound("Shock")
	self:EmitSound("debris/beamstart2.wav", 100, 160)
	timer.Simple(1, function()
		for k, v in pairs(beams) do if ValidEntity(k) then k:Remove() end; if ValidEntity(v) then v:Remove() end end
		if !ValidEntity(self) or (!ValidEntity(self.enemy) and !self.possessed) then self.attacking = false return end
		local bolt = ents.Create("voltigore_bolt")
		bolt:SetPos( self:GetPos() +self:GetForward() *40 +self:GetUp() *30 )
		bolt:SetAngles(self:GetAngles())
		bolt.owner = self
		bolt:SetMoveCollide( 3 )
		if !self.possessed then bolt.enemy = self.enemy end
		bolt:SetOwner( self )
		bolt:Spawn()
		local phys = bolt:GetPhysicsObject()
			if !self.possessed then phys:ApplyForceCenter( ( self.enemy:GetPos() - self:GetPos() ):GetNormal() * 4000 )
			else phys:ApplyForceCenter( self:GetForward() * 4000 ) end
...
function ENT:CanRangeAttack()
	if self.enemy:GetPos():Distance(self:GetPos()) <= self.RangeDistance and self:Visible(self.enemy) then return true else return false end
end
		...
		if( ValidEntity( self.enemy ) and self.enemy:GetPos():Distance( self:GetPos() ) <= self.closest_range ) then
			...
			if self:HasCondition( 10 ) and !self:HasCondition( 42 ) and (self:CanRangeAttack() or self:CanMeleeAttack()) then
				...
				elseif dist <= self.MeleeDistance then
					self:PlayRandomSound("Attack")
					if math.random(1,2) == 1 then
						self:AttackMelee(schdMeleeAttack_a, 125, sk_voltigore_slash_value /1.8, "voltigore", Angle( 10, -7, 0 ), 0.55, 0, "zombie/claw_strike" ..math.random(1,3).. ".wav" )
					else
						self:AttackMelee(schdMeleeAttack_b, 125, sk_voltigore_slash_value, "voltigore", Angle( 10, 0, 0 ), 0.96, 0, "zombie/claw_strike" ..math.random(1,3).. ".wav" )
					end
				end
