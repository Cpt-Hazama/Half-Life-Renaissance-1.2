...
ENT.Model = "models/w_penguin.mdl"
ENT.MinDistance		= 235
ENT.CheckWorld = true
...
schdAttack:EngTask( "TASK_PLAY_SEQUENCE_FACE_ENEMY", ACT_JUMP )
...
function ENT:Initialize()
	...
	if penguingun_owner then
		self.owner = penguingun_owner
	end
	if !self.dontdestruct then
		if !self.blasttime then
			timer.Create( "self_explode_timer" .. self:EntIndex(), sk_penguin_delay_value, 1, function() self:TakeDamage( self:Health(), self ) end )
		else
...
function ENT:OnCondition( iCondition )
	...
	elseif( self:HasCondition( 8 ) and !self:HasCondition( 13 ) ) then
		...
		timer.Destroy( "wandering_timer" .. self.Entity:EntIndex( ) )
		timer.Destroy( "penguin_hunt_timer" .. self:EntIndex() )
		self.timer_created = false
		...
		for k, v in pairs(victim) do
			if( !table.HasValue( enemy_table, v ) ) then
				if( ( ( ( v:IsPlayer() and v:Alive() ) or v:IsNPC() ) and ( self:Disposition( v ) == 1 or self:Disposition( v ) == 2 ) ) or v:GetClass() == "prop_physics" ) then
					...
					if v:IsNPC() and v:Health() - sk_penguin_melee_value <= 0 then
						self.killicon_ent = ents.Create( "sent_killicon" )
						self.killicon_ent:SetKeyValue( "classname", "sent_killicon_penguin" )
					...
					if self.owner and self.owner:IsPlayer() then
						if( v:Health() > 0 and v:Health() - sk_penguin_melee_value <= 0 ) then
							self.owner:AddFrags( 1 )
						end
						v:TakeDamage( sk_penguin_melee_value, self.owner, self.attack_inflictor )  
					else
						v:TakeDamage( sk_penguin_melee_value, self, self.attack_inflictor )  
					end
...
function ENT:OnTakeDamage(dmg)
	...
	if ValidEntity(self.attacker) and !self.enemy and self.enemy_memory and ( !self.WaterMonster or ( self.WaterMonster and self.attacker:WaterLevel() > 0 ) ) then
	...
	if ( self:Health() <= 0 and !self.dead ) then
		...
		if( self.attacker:GetClass() != "npc_barnacle" and !dmg:IsDamageType( DMG_DISSOLVE ) ) then
			...
			local function penguin_explode()
				...
				if ValidEntity( self.owner ) then
					util.BlastDamage( self, self.owner, self:GetPos(), 180, self.blastdmg )
				else
					util.BlastDamage( self, self, self:GetPos(), 180, self.blastdmg )
				end
				...
				for k, v in pairs( dmg_ents ) do
					if( ( ( ( v:IsPlayer() and v:Alive() ) or v:IsNPC() ) and ( self:Disposition( v ) == 1 or self:Disposition( v ) == 2 ) ) or v:GetClass() == "prop_physics" ) then
						...
						if self.owner and self.owner:IsPlayer() then
							if( v:Health() > 0 and v:Health() - sk_penguin_blast_value <= 0 ) then
								self.owner:AddFrags( 1 )
							end
			...
			timer.Create( "explode_timer" .. self:EntIndex(), 0.5, 1, penguin_explode )
			//self:SetNPCState( NPC_STATE_DEAD )
			//self:Remove()
			...
			elseif( ( self.following and self.enemy:GetPos():Distance( self.follow_target:GetPos() ) < 800 ) or !self.following ) then
				...
				if( self:GetPos():Distance( self.enemy:GetPos() ) < 1600 ) then
					...
					if( !self.hunt_timer_created ) then
						...
						timer.Create( "penguin_hunt_timer" .. self:EntIndex(), math.random(0.1,0.3), 1, penguin_hunt )
					end
				end
			end
