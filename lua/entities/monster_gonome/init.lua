...
ENT.possess_viewheight = 84

ENT.DSounds = {}
ENT.DSounds["Attack"] = {"gonome/gonome_melee1.wav", "gonome/gonome_melee2.wav"}
ENT.DSounds["Jump"] = {"gonome/gonome_jumpattack.wav"}
ENT.DSounds["Death"] = {"gonome/gonome_death2.wav", "gonome/gonome_death3.wav", "gonome/gonome_death4.wav"}
ENT.DSounds["Pain"] = {"gonome/gonome_pain1.wav", "gonome/gonome_pain2.wav", "gonome/gonome_pain3.wav", "gonome/gonome_pain4.wav"}
ENT.DSounds["Idle"] = {"gonome/gonome_idle1.wav", "gonome/gonome_idle2.wav", "gonome/gonome_idle3.wav"}
...
schdRangeAttack:EngTask( "TASK_STOP_MOVING", 0 )
schdRangeAttack:EngTask( "TASK_PLAY_SEQUENCE_FACE_ENEMY", ACT_RANGE_ATTACK2 )
...
function ENT:Initialize()
	...
	if !self.health then
		self:SetHealth(sk_gonome_health_value)
	end
	
	if self.triggertarget and self.triggercondition == "3" then self.starthealth = self:Health() end

	self:SetUpEnemies( {"npc_fastzombie_torso", "npc_fastzombie",  "npc_poisonzombie", "npc_zombie", "npc_zombie_torso", "npc_zombine", "npc_headcrab", "npc_headcrab_black", "npc_headcrab_poison", "npc_headcrab_fast", "monster_babycrab", "monster_headcrab", "monster_bigmomma", "monster_zombie", "monster_zombie_barney", "monster_zombie_soldier"} )
	...
	if self.possessed then
		...
		if !self.attacking and ( !self.possession_allowdelay or ( self.possession_allowdelay and CurTime() > self.possession_allowdelay ) ) then
			...
			elseif self.master:KeyDown( 2048 ) then
				...
				if rand == 1 then
					self:AttackMelee(schdMeleeAttack_a, self.MeleeDistance, sk_gonome_slash_value, "gonome", Angle( -5, -14, 0 ), 0.5, 0, "zombie/claw_strike" ..math.random(1,3).. ".wav", {1, "zombie/claw_strike" ..math.random(1,3).. ".wav", Angle( -5, 14, 0 )} )
				else
					self:AttackMelee(schdMeleeAttack_b, self.MeleeDistance, sk_gonome_slash_value *1.25, "gonome", Angle(4,0,0), 0.3, 0, "" )
				end
			end
		end
	end
...
function ENT:Attack_Range( poss )
	...
	timer.Simple(0.94, function()
		if !ValidEntity(self) or (!poss and !ValidEntity(self.enemy)) then return end
		for i=1, 4 do
			local spitball = ents.Create("gonome_acid")
			spitball:SetParent(self)
			spitball.owner = self
			spitball.sleeping = true
			spitball:SetOwner( self )
			spitball:Spawn()
			spitball:Fire("SetParentAttachment", "0", 0)
			table.insert(spitballs,spitball)
		end
	end)
	
	local function lAttak( tar )
		if !ValidEntity(self) or self.dead or (!poss and (!ValidEntity(self.enemy) or !poss and !tar:IsValid())) then for k, v in pairs(spitballs) do if ValidEntity(v) then v:Remove() end end; self.attacking = false; return end
		local pos = self:GetAttachment(self:LookupAttachment("0"))["Pos"]
		local FireTrace
		if poss then
			FireTrace = self:GetForward() *350 +Vector(0,0,12)
		else
			FireTrace = ((self.enemy:GetPos() + Vector(0,0,35)) - pos)
		end
		...
		timer.Simple(0.5,function() if ValidEntity(self) then self.attacking = false end end)
		if poss then self.possession_allowdelay = CurTime() +1.4 end
	end
	timer.Create( "range_attack_end_timer" .. self.Entity:EntIndex( ), 1.65, 1, lAttak, self.enemy )
end
...
function ENT:SelectSchedule()
	...
	if( ( self.FoundEnemy or self.FoundEnemy_fear ) and !self.attacking and !self.possessed and convar_ai == 0 ) then
		...
		if( self.enemy and ValidEntity( self.enemy ) and self.enemy:GetPos():Distance( self:GetPos() ) <= self.closest_range ) then
			...
			if self:HasCondition( 10 ) and !self:HasCondition( 42 ) and (dist <= self.MeleeDistance or (dist <= self.RangeDistance and self.allow_range_attack)) then
				...
				if dist <= self.RangeDistance and dist > self.MeleeDistance then
					self:StartSchedule( schdRangeAttack )
					self:Attack_Range()
