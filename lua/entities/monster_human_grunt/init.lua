...
function ENT:Initialize()
	...
	self:SetUpEnemies( {"monster_human_assassin", "monster_sentry", "monster_hwgrunt"} )
...
function ENT:Think()
	...
	self:TurnToEnemyXR(-90,45)
	self:ValidateRelationships()
	
	if self.possessed then
		if !self:PossessView() then return end
		self:Possess_SetViewVector()
		if !self.attacking and !self.reloading and ( !self.possession_allowdelay or ( self.possession_allowdelay and CurTime() > self.possession_allowdelay ) ) then
...
function ENT:Attack_ar( poss )
	...
	local function fire()
		...
		local enemy_pos
		if !poss then
			enemy_pos = self.enemy:GetPos() +self:GetPositionAnomaly(self.enemy:GetClass())
		else
