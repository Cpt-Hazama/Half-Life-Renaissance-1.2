...
function ENT:Initialize()
	...
	self:SetUpEnemies( {"monster_human_grunt", "monster_sentry", "monster_hwgrunt"} )
...
function ENT:Think()
	...
	self:TurnToEnemyXR(-23,45)
	self:ValidateRelationships()
	
	if self.possessed then
		if !self:PossessView() then return end
		self:Possess_SetViewVector()
		if !self.attacking and !self.jumping and ( !self.possession_allowdelay or ( self.possession_allowdelay and CurTime() > self.possession_allowdelay ) ) then
