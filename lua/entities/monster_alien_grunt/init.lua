...
ENT.possess_viewheight = 80

ENT.DSounds = {}
ENT.DSounds["Attack"] = {"agrunt/ag_attack1.wav", "agrunt/ag_attack2.wav", "agrunt/ag_attack3.wav"}
ENT.DSounds["Alert"] = {"agrunt/ag_alert1.wav", "agrunt/ag_alert2.wav", "agrunt/ag_alert3.wav", "agrunt/ag_alert4.wav", "agrunt/ag_alert5.wav"}
ENT.DSounds["Death"] = {"agrunt/ag_die1.wav", "agrunt/ag_die2.wav", "agrunt/ag_die3.wav", "agrunt/ag_die4.wav", "agrunt/ag_die5.wav"}
ENT.DSounds["Pain"] = {"agrunt/ag_pain1.wav", "agrunt/ag_pain2.wav", "agrunt/ag_pain3.wav", "agrunt/ag_pain4.wav", "agrunt/ag_pain5.wav"}
ENT.DSounds["Idle"] = {"agrunt/ag_idle1.wav", "agrunt/ag_idle2.wav", "agrunt/ag_idle3.wav", "agrunt/ag_idle4.wav", "agrunt/ag_idle5.wav"}
...
function ENT:Initialize()
	...
	self:SetUpEnemies( {"monster_alien_controller", "monster_alien_slave", "monster_nihilanth"} )
	...
	if self.possessed then
		...
		if !self.attacking and ( !self.possession_allowdelay or ( self.possession_allowdelay and CurTime() > self.possession_allowdelay ) ) then
			...
			elseif self.master:KeyDown( 2048 ) then
				...
				self:AttackMelee(schdMeleeAttack, self.MeleeDistance, sk_agrunt_slash_value, "agrunt", Angle( -5, 20, 0  ), 0.65, 0.5, "npc/zombie/claw_strike" ..math.random(1,3).. ".wav" )
...
function ENT:Attack_Range( poss )
	...
	local function hornet_spawn()
		...
		hornet:SetPos( self:GetAttachment(1)["Pos"] )
			...
			if( self.enemy:GetPos():Distance( Pos ) < self.RangeDistance and self.enemy:GetPos():Distance( Pos ) > self.MeleeDistance and self:HasCondition( 10 ) and !self:HasCondition( 42 ) ) then
				...
				local function get_victim_pos()
					if( self.enemy:IsValid() == true ) then
						self.victim_pos = self.enemy:GetPos() +self:GetPositionAnomaly(self.enemy:GetClass())
