...
function SWEP:ShootZap( damage, force, beamwidth, scorchdecal )
	...
	local attachpos = self.Owner:GetViewModel():GetAttachment( self.Owner:GetViewModel():LookupAttachment( "0" ) )["Pos"]
	attachpos.z = attachpos.z +self.Owner:WorldToLocal(self.Owner:GetEyeTrace( ).StartPos).z
...
function SWEP:CreateBeam( StartPos, EndPos, beamwidth )
	...
	if self.Owner:IsPlayer() then
		self.beamtarget:SetParent( self.Owner:GetViewModel() )
	else
		self.beamtarget:SetParent( self.Owner )
	end
	self.beamtarget:Spawn()
	self.beamtarget:Activate()
	if self.Owner:IsPlayer() then
		self.beamtarget:Fire( "SetParentAttachment", "0", 0 )
	else
		self.beamtarget:Fire( "SetParentAttachment", "anim_attachment_LH", 0 )
	end
	...
	self.beam:Spawn()
	self.beam:Activate()
	self.beam:Fire( "Kill", "", 0.19 )
	self.beamtarget:Fire( "Kill", "", 0.19 )
