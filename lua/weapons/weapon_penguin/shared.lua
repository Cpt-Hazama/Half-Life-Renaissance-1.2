...
SWEP.ViewModel = "models/weapons/v_penguin.mdl"
SWEP.WorldModel = "models/w_penguinnest.mdl"
...
function SWEP:Initialize()
	...
	if CLIENT or ( SERVER and ValidEntity( self.Owner ) ) then return end
		...
		self.swep_item:SetModel( "models/w_penguinnest.mdl" )
...
function SWEP:PrimaryAttack()
	...
	penguingun_owner = self.Owner
	local penguin = ents.Create( "monster_penguin" )
	local penguin_spawnpos = self:LocalToWorld( Vector( 40, 0, 0 ) )
	if self.owner:IsPlayer() then
		penguin_spawnpos.z = self.tr.StartPos.z -16
	end
	penguin:SetPos( penguin_spawnpos )
	local penguin_angles = self.owner:GetAngles()
	penguin_angles.p = 0
	penguin_angles.r = 0
	penguin:SetAngles( penguin_angles )
	penguin:Spawn()
	penguin:Activate()
	penguin:Fire( "SetSquad", tostring(self.Owner) .. "_penguinsquad", 0.1 )
	penguingun_owner = nil
	constraint.NoCollide( penguin, self.Owner, 0, 0 )
	local pl_eye_ang = self.Owner:EyeAngles()
	local penguin_throwpos = self.Owner:GetShootPos() + pl_eye_ang:Right() * 5 - pl_eye_ang:Up() * 7
	local penguin_throwpos_f = ( util.TraceLine( util.GetPlayerTrace( self.Owner ) ).HitPos - penguin_throwpos ):GetNormalized()
	penguin:SetVelocity( penguin_throwpos_f * 500 )
